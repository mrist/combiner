...
public class EvaluationBackup {
	...
	public ResultStats getFirstResultWithAccuracyThreshhold(Relations relations, ArrayList<Phrase> claims,
			double threshhold) {
		ArrayList<ProbabilityNode> pNodes = initializeRelationProbablilityNodes(relations);
		...
		for (Phrase claim : claims) {
			...
			do {
				results = claim.getPatternTarget((RelationPatternNode) pNodes.get(i).getTree());
				i++;
			} while ((pNodes.get(i).getAccuracy() > threshhold && results.isEmpty()) && i < pNodes.size());
			if (results.isEmpty()) {
				r.addNone();
			}
			else {
	...
	public ResultStats get10FoldResultsWithThreshhold(Relations relations, int limit, int noise, int topX,
			double threshhold) {
		ArrayList<Relations> relationsList = relations.split10();
		for (int i = 0; i < relationsList.size(); i++) {
			Relations testRelations = new Relations();
			...
			System.out.println(relationProbNodes.size());
			System.out.println(phraseProbNodes.size());
				...
				do {
					...
					if (resultWords != null && !resultWords.isEmpty()) {
						...
						if (currentAccuracy >= adjustedAccuracy) {
							results = getFirstResultForPhrase(phraseProbNodes, resultWords);// getResultForPhraseWithThreshhold(phraseProbNodes,
																							// resultWords,
																							// threshhold);
							for (Result result : results) {
								result.setAspect(claim);
								result.setRelationProbabilityNode(relationProbNodes.get(counter));
								result.checkResult(noise);
								allResults.add(result);
							}

							r.setRelationProbabilityNode(relationProbNodes.get(counter));
	...
	public ResultStats get10FoldResultsWithParameters(Relations relations, int limit, int topX, int noise,
		...
		for (int i = 0; i < relationsList.size(); i++) {
			...
			Collections.sort(phraseProbNodes, comp);
			for (Phrase claim : testRelations.getAspects()) {
				double adjustedAccuracy = 0.0;
				double currentAccuracy = 0.0;
				int counter = -1;
				...
				do {
					...
					if (currentResult.hasTargetNodes()) {
						...
						if (currentAccuracy >= adjustedAccuracy) {
							currentResult.setSortingValue(currentAccuracy);

							currentResult = getPhraseProbResult(phraseProbNodes, currentResult);
	...
	public ResultStats get10FoldResultsWithParametersSRL(Relations relations, int limit, int topX, int noise,
			double threshhold, Comparator<ProbabilityNode> comp) {
	...
	public ResultStats get10FoldResultsWithParametersComb(Relations relations, int limit, int topX, int noise,
			double threshhold, Comparator<ProbabilityNode> compRel, Comparator<ProbabilityNode> compPhrase) {
		...
		for (int i = 0; i < relationsList.size(); i++) {
			...
			Collections.sort(relationProbNodes, compRel);
			Collections.sort(phraseProbNodes, compPhrase);
			for (Phrase claim : testRelations.getAspects()) {
				double adjustedAccuracy = 0.0;
				double currentAccuracy = 0.0;
				int counter = -1;
	...
	public ResultStats get10FoldRelationResultsWithThreshhold(Relations relations, int limit, int topX,
			double threshhold) {
		ArrayList<Relations> relationsList = relations.split10();
		for (int i = 0; i < relationsList.size(); i++) {
			Relations testRelations = new Relations();
			...
			Collections.sort(relationProbNodes);
			System.out.println("support: " + relationProbNodes.get(0).getTotalSupport());
			System.out.println("total: " + relationProbNodes.get(0).getTotalSupport());
			...
			System.out.println("");
			evalutateTargetExitNodesAt(allResults, testRelations.all().size(), topX);

		}
		return new ResultStats();
	}

	public ResultStats get10FoldRelationResultsWithParameters(Relations relations, int limit, int topX, int noice,
			double threshhold, Comparator<ProbabilityNode> comp) {
		ResultStats all = new ResultStats();
		Boolean newResult;
		RelationPatternNode currentPattern;
	...
	public ResultStats get10FoldPhraseResultsWithThreshhold(Relations relations, int limit, int noise) {
		ArrayList<Relations> relationsList = relations.split10();
		...
		for (int i = 0; i < relationsList.size(); i++) {
			...
			Collections.sort(phraseProbNodes);
			System.out.println("support: " + phraseProbNodes.get(0).getTotalSupport());

			total += testRelations.getStatements().size();
			...
			System.out.println("");
			evalutateCorrectResult(allResults, total);
	...
	public ResultStats get10FoldPhraseResultsWithParameters(Relations relations, int limit, int topX, int noise,
			double threshhold, Comparator<ProbabilityNode> comp) {
		ResultStats all = new ResultStats();
		Relations clearedRelation = relations.getClearedRelations();
		ArrayList<Relations> relationsList = clearedRelation.split10();
	...
	@Deprecated
	public Result getPhrasesSRL(Result input) {
		ArrayList<WordNode> subTreeNodes = new ArrayList<WordNode>();
		for (WordNode exitNode : input.getTargetExitNodes()) {
			subTreeNodes.addAll(exitNode.getSubtreeNodes());
		}
		input.setPhraseNodes(subTreeNodes);
		return input;
	}

	public ArrayList<Result> getResultForPhraseWithThreshhold(ArrayList<ProbabilityNode> pNodes,
			ArrayList<WordNode> words, double threshhold) {
		...
		do {
			...
			for (WordNode exitNode : words) {
				...
				resultWordNode = exitNode.getAllNodes((PhrasePatternNode) currentProbNode.getTree());
				if (resultWordNode != null && !resultWordNode.isEmpty() && !words.containsAll(resultWordNode)) {

					currentAccuracy = currentProbNode.getAccuracy();
					if (adjustedAccuracy == 0.0) {
						adjustedAccuracy = currentAccuracy;
					}
					if (currentAccuracy > adjustedAccuracy - (adjustedAccuracy * threshhold)) {
	...
	public ResultStats getFirstPhrasesAboveAccuracyThreshhold(ArrayList<Phrase> phrasesTraining,
			ArrayList<Phrase> phrasesTest, double threshhold) {
		ArrayList<ProbabilityNode> pNodes = initializePhraseProbablilityNodes(phrasesTraining);
		...
		for (Phrase phrase : phrasesTest) {
				...
				for (WordNode exitNode : phrase.getExitNodes()) {
					...
					if (results != null && !results.isEmpty()) {
						...
						if (currentAccuracy > adjustedAccuracy - (adjustedAccuracy * threshhold)) {
							...
							if (results.containsAll(phrase.getWords())) {
								r.addHit();
							}
							else {
								r.addMiss();
			...
			} while (i <= pNodes.size() - 2 && results.isEmpty());
	...
	public void latexTablePrint(Relations relations, ArrayList<Phrase> claims) {
		int limit = 1;
		int topX = 500000;
		int noise = 0;
		double threshold = .1;
	...
	public ResultStats get10FoldRelationResultsFinal(Relations relations, int limit, int topX, int noice,
	...
	public ResultStats get10FoldPhraseResultsFinal(Relations relations, int limit, int topX, int noise,
			double threshhold, CompType comp) {
		ResultStats all = new ResultStats();
		Relations clearedRelation = relations.getClearedRelations();
		ArrayList<Relations> relationsList = clearedRelation.split10();
	...
	public ResultStats get10FoldResultsFinal(Relations relations, int limit, int topX, int noise, double threshhold,
			CompType compRelation, CompType compPhrase) {
	...
	public ArrayList<Result> get10FoldResultListFinal(Relations relations, int limit, int topX, int noise,
